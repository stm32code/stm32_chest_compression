...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"hight\":\"%03d\",", (U16)Data_init.hight); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"hight_num\":\"%03d\",", (U16)device_state_init.hight_num); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"HZ\":\"%03d\"", (U16)device_state_init.hight_hz); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
...
unsigned char Mqtt_Flage_Info(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Huim_Value\":\"%d\"", threshold_value_init.threshold_humi); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
